---
title: "Week 12 Worklog"
date: 2024-01-01
weight: 2
chapter: false
pre: " <b> 1.12. </b> "
---


### Week 12 Objectives:

- Complete all work and the project during the internship.

### Summary of tasks completed during the week:

| Day | Task | Start Date | End Date | Source |
| --- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------ | --------------- | ----------------------------------------- |
| 2 | - Complete the Diagram table | 06/07/2026 | 06/07/2026 | |
| 3 | - Learn about and upload the entire workshop to Github | 07/07/2026 | 07/07/2026 | |
| 4 | - Write and complete the proposal | 08/07/2026 | 08/07/2026 | |
| 5 & 6 | - Complete all requirements to request internship acceptance letter | 09/07/2026 | 10/07/2026 | |

### Week 12 Achievements:
### Day 2 -> Day 6:

1. **Complete Diagram table:**

![Cloud Architecture Diagram](/images/1-Worklog/1.12-Week12/02-week12-diagram.png)

**System description:**
- Cloud Nexus is a network security simulation and analysis platform for cybersecurity and infrastructure architects.
- The system allows users to visually design network diagrams, then use AI (Google Gemini) to automatically detect vulnerabilities, simulate attack paths, and propose defense measures.

2. **Write and complete Proposal:**

-- 2 of 4 --

3. **Upload entire workshop to Github:**

4. **Complete all requirements to request internship acceptance letter:**

-- 4 of 4 --


